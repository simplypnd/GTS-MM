...
export function RegisterForm() {
  ...
  const [referralLookupError, setReferralLookupError] = useState(false);
    ...
    (async () => {
      try {
        ...
        if (data.found && data.display_name) {
          ...
          setReferralLookupError(false);
        } else {
          setReferrerName(null);
          setReferralLookupError(true);
        }
      } catch {
        if (!cancelled) setReferralLookupError(true);
      }
    })();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const effectiveReferral =
      referralCode?.trim() || getStoredReferralCode()?.trim();
    if (effectiveReferral && !referralLookupError) {
      signUpMeta.referral_code = effectiveReferral;
    }
  ...
  return (
    <Card className="mx-auto max-w-md">
      <CardHeader>
        ...
        {referralCode && referralLookupError && (
          <p className="text-sm text-amber-700 dark:text-amber-400">
            Referral link not recognized—you can still register.
          </p>
        )}
      </CardHeader>
      <CardContent>
